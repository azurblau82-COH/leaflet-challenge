...
function createQuakes(response) {
    ...
    //add new function to fetch fault line data
...
function addFaults(response){
    ...
    for (var index = 0; index < faults.length; index++) {
        ...
        for (var index2 = 0; index2 < fault.geometry.coordinates.length; index2++){
        var faultlineX = [fault.geometry.coordinates[index2][1], fault.geometry.coordinates[index2][0]];
        faultline.push(faultlineX)
        }
        L.polygon(faultline).addTo(map);
        faultlines.push(faultline);
    }
